...
# define X and y
X, y = datasets.make_regression(n_samples=10, n_features=4)
...
plt.scatter(
    y[350:442], fit_obj.predict(X[350:442, :]), color="red"
)
plt.scatter(
    y[350:442],
    fit_obj2.predict(X[350:442, :]),
    color="blue",
)
plt.scatter(
    y[350:442],
    fit_obj3.predict(X[350:442, :]),
    color="green",
)
plt.scatter(
    y[350:442],
    fit_obj4.predict(X[350:442, :]),
    color="black",
)
plt.scatter(
    y[350:442],
    fit_obj5.predict(X[350:442, :]),
    color="magenta",
)
plt.scatter(
    x=np.sort(y[350:442]),
...
print(
    np.sqrt(
        (
            (fit_obj.predict(X[350:442, :]) - y[350:442])
            ** 2
        ).mean()
    )
)
...
print(
    np.sqrt(
        (
            (fit_obj2.predict(X[350:442, :]) - y[350:442])
            ** 2
        ).mean()
    )
)
...
print(
    np.sqrt(
        (
            (fit_obj3.predict(X[350:442, :]) - y[350:442])
            ** 2
        ).mean()
    )
)
...
## Example 2 - Custom - n_hidden_features=100, 500 -----
...
## Example 3 - BayesianRVFL2 - n_hidden_features=5 -----

# create object BayesianRVFL2
fit_obj = ns.BayesianRVFL2(
    n_hidden_features=5,
    direct_link=False,
    activation_name="tanh",
    n_clusters=3,
...
plt.plot(
    x, fit_obj2.predict(X[350:442, :])[0], color="blue"
)
plt.plot(
    x, fit_obj3.predict(X[350:442, :])[0], color="green"
)
...
print(
    np.sqrt(
        (
            (fit_obj.predict(X[350:442, :])[0] - y[350:442])
            ** 2
        ).mean()
    )
)
...
plt.plot(
    x, fit_obj2.predict(X[350:375, :])[0], color="blue"
)
plt.plot(
    x, fit_obj3.predict(X[350:375, :])[0], color="green"
)
plt.title("preds vs test set obs")
plt.xlabel("x")
plt.ylabel("preds")
...
plt.plot(
    x, fit_obj4.predict(X[350:442, :])[0], color="yellow"
)
plt.title("preds vs obs")
plt.xlabel("x")
plt.ylabel("preds")
...
plt.plot(
    x, fit_obj4.predict(X[350:375, :])[0], color="green"
)
plt.title("preds vs test set obs")
plt.xlabel("x")
plt.ylabel("preds")
...
np.sqrt(
    metrics.mean_squared_error(
        y[100:125], layer1_regr.predict(X[100:125, :])
    )
)


# layer 2 ----
layer2_regr = ns.Custom(
    obj=layer1_regr,
    n_hidden_features=3,
    direct_link=True,
    bias=True,
...
## Example 8 - Multivariate time series -----
...
obj_MTS = ns.MTS(
    regr4, lags=1, n_hidden_features=5, bias=False
)
obj_MTS.fit(X)
...
obj_MTS4 = ns.MTS(
    ...
    n_hidden_features=2,
    bias=True,
    type_scaling=("minmax", "std", "minmax"),
)
obj_MTS4.fit(X)
...
obj_MTS = ns.MTS(
    regr4, lags=1, n_hidden_features=2, bias=True
)
obj_MTS.fit(Y)
print(obj_MTS.predict())
...
obj_MTS3 = ns.MTS(
    regr6, lags=1, n_hidden_features=3, bias=True
)
obj_MTS3.fit(X)
print(obj_MTS3.predict())
