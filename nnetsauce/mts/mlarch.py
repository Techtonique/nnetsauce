...
class MLARCH(MTS):
    ...
    def __init__(self, model_mean, model_sigma, model_residuals):
        assert isinstance(
            model_mean, MTS
        ), "model_mean must be an object of class nnetsauce.MTS"
        assert isinstance(
            model_sigma, MTS
        ), "model_sigma must be an object of class nnetsauce.MTS"
        assert isinstance(
            model_residuals, MTS
        ), "model_residuals must be an object of class nnetsauce.MTS"
        assert (
            model_sigma.type_pi.startswith("scp")
            and model_sigma.replications is not None
        ), "for now, the models must be conformalized, i.e type_pi must start with 'scp' and replications must be an integer"
        assert (
            model_residuals.type_pi.startswith("scp")
            and model_residuals.replications is not None
        ), "for now, the models must be conformalized, i.e type_pi must start with 'scp' and replications must be an integer"
    ...
    def fit(self, y, **kwargs):
        ...
        self.mean_residuals_wilcoxon_test_ = stats.wilcoxon(
            self.model_mean.residuals_
        )
        # KPSS test for stationarity on residuals
        self.mean_residuals_kpss_test_ = kpss(
            self.model_mean.residuals_, regression="c"
        )
        self.model_sigma.fit(
            np.log(self.model_mean.residuals_.reshape(-1, 1) ** 2)
        )
        # n//2 here because the model is conformalized
        fitted_sigma = (
            self.model_sigma.residuals_
            + np.log(self.model_mean.residuals_**2)[(n // 2):, :]
        )
        # standardized residuals
        self.standardized_residuals_ = self.model_mean.residuals_[
            (n // 2):, :
        ...
        n_params = (
            self.model_mean.n_hidden_features
            + 1  # mean model
            + self.model_sigma.n_hidden_features
            + 1  # sigma model
            + self.model_residuals.n_hidden_features
            + 1
        )  # residuals model
    ...
    def predict(self, h=5, level=95):
        ...
        for i in range(len(sims_z)):
            f.append(
                mean_forecast
                + sims_z[i].values.ravel()
                * np.sqrt(np.exp(sims_sigma[i].values.ravel()))
            )
        ...
        return DescribeResult(mean_f, f, lower_bound, upper_bound)
