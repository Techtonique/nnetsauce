...
class MultiOutputMTS(MTS):
    ...
    def fit(self, X, xreg=None, **kwargs):
        ...
        try:
            ...
            raise ValueError(
                "MultiOutputMTS requires multivariate input (n_samples, n_series)"
            )

        if self.init_n_series_ < 2:
            raise ValueError(
                f"MultiOutputMTS requires at least 2 series, got {self.init_n_series_}"
            )
        ...
        y_means_array = np.array(
            [np.mean(self.y_[:, i]) for i in range(self.init_n_series_)]
        )
        ...
        self.centered_y_is_ = [
            centered_y_all[:, i] for i in range(self.init_n_series_)
        ]
        ...
        class VectorizedWrapper:
            ...
            def predict(self, X, **kw):
                ...
                if len(preds.shape) == 1:
                    ...
                    return preds[
                        :, self.series_idx: self.series_idx + 1
                    ].flatten()
