...
class DiscreteTokenMTS(MTS):
    ...
    **mts_kwargs : dict
    ...
    def _validate_vocabulary(self):
        ...
        # Check shape
        assert (
            self.vocab_original.ndim == 2
        ), "vocab must be 2D array (vocab_size, n_series)"
        assert (
            self.vocab_original.shape[0] > 0
        ), "vocab must have at least one token"

        # Check for NaN/Inf
        if np.any(np.isnan(self.vocab_original)) or np.any(
            np.isinf(self.vocab_original)
        ):
    ...
    def _vectorized_map_to_tokens(self, continuous_preds):
        ...
        if self.normalize_vocab:
            continuous_preds = (
                continuous_preds - self.vocab_mean_
            ) / self.vocab_std_
    ...
    def predict(
        ...
        level=95,
        quantiles=None,
        return_discretization_error=False,
        **kwargs,
    ):
        """
        ...
        **kwargs : dict
        ...
        continuous_result = super().predict(
            h=h, level=level, quantiles=quantiles, **kwargs
        )

        # FIXED: Robust type detection using duck typing
        if hasattr(continuous_result, "_fields"):  # Namedtuple
            if (
                hasattr(continuous_result, "sims")
                and continuous_result.sims is not None
            ):
                # Simulation-based forecast
                return self._discretize_simulations(
                    continuous_result.sims, return_discretization_error
                )
                ...
                return self._discretize_dataframe(
                    continuous_result.mean, return_discretization_error
                )
        elif isinstance(continuous_result, pd.DataFrame):
            # Deterministic forecast
            return self._discretize_dataframe(
                continuous_result, return_discretization_error
            )
        else:
    ...
    def _discretize_dataframe(self, df, return_error=False):
        ...
        if self.return_mode == "probs":
            ...
            columns = ["token_id"] + [
                f"dim_{i}" for i in range(self.vocab.shape[1])
            ]
            ...
            result_df = pd.DataFrame(
                result.reshape(-1, 1), index=df.index, columns=["token_id"]
            )
        else:  # 'token_vector'
            result_df = pd.DataFrame(
                result,
    ...
    def _discretize_simulations(self, sims, return_error=False):
        ...
        for sim_df in sims:
            ...
            if self.return_mode == "probs":
                ...
                discrete_df = pd.DataFrame(
                    result, index=sim_df.index, columns=columns
                )
    ...
    def predict_top_k(self, h=5, k=5, **kwargs):
        ...
        **kwargs : dict
        ...
        return pd.DataFrame(np.column_stack(data), index=index, columns=columns)
    ...
    def predict_token_distribution(self, h=5, replications=100, **kwargs):
        ...
        **kwargs : dict
        ...
        # Compute entropy
        epsilon = 1e-10
        entropy = -np.sum(token_freqs * np.log(token_freqs + epsilon), axis=1)
        ...
        entropy_series = pd.Series(entropy, index=index, name="entropy")

        mode_df = pd.DataFrame(mode_tokens, index=index, columns=["mode_token"])
    ...
    def get_token_neighbors(self, token_id, k=5):
        ...
        return pd.DataFrame(
            {"neighbor_id": sorted_indices, "distance": dists[sorted_indices]}
        )
    ...
    def print_vocabulary_report(self):
        ...
        if report["condition_number"] > 1000:
            print(
                "  ⚠️  WARNING: High condition number may indicate redundant tokens"
            )
