...
class RandomFourierFeaturesRidge(BaseEstimator, RegressorMixin):
    ...
    def _compute_random_features(
        ...
        if self.include_bias:
            features = jnp.concatenate([jnp.ones((X.shape[0], 1)), features], axis=1)

        return features

    def _init_random_weights(self, input_dim: int) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        W = random.normal(
            subkey,
            shape=(input_dim, self.n_features)
    ...
    def _fit_standard(self, Phi: jnp.ndarray, y: jnp.ndarray) -> None:
        ...
        weights, residuals, rank, s = jnp.linalg.lstsq(Phi_aug, y_aug, rcond=None)
    ...
    def _fit_bayesian(self, Phi: jnp.ndarray, y: jnp.ndarray,
                     noise_variance: Optional[float] = None) -> None:
    ...
    def predict(
        ...
        if self.method != "bayesian":
            raise ValueError("Uncertainty quantification only available for Bayesian method")

        # Predictive variance
        if return_cov:
            # Full predictive covariance
            # Σ_pred = (1/β) * I + Φ @ S_N @ Φᵀ
            pred_cov = (1.0 / self.beta) * jnp.eye(Phi.shape[0]) + Phi @ self.S_N @ Phi.T
            return y_pred, pred_cov
        else:
            # Diagonal of predictive covariance (standard deviations)
            # σ²_pred = (1/β) + diag(Φ @ S_N @ Φᵀ)
            var_diag = (1.0 / self.beta) + jnp.sum((Phi @ self.S_N) * Phi, axis=1)
    ...
    def sample_posterior(
        ...
        for i in range(n_samples):
            ...
            noise = random.normal(subkey2, shape=pred_mean.shape) / jnp.sqrt(self.beta)
    ...
    def log_marginal_likelihood(self) -> float:
        ...
        """
        if self.method != "bayesian":
            raise ValueError("Log marginal likelihood only available for Bayesian method")
...
class RandomFourierFeaturesRidgeGCV(RandomFourierFeaturesRidge):
    ...
    """    
    def __init__(self, n_features: int = 100, gamma: float = 1.0,
                 alpha: Optional[float] = None, include_bias: bool = True,
                 random_seed: int = 42):
    ...
    def _compute_gcv(self, alpha: float, s_sq: jnp.ndarray, U: jnp.ndarray, 
                     y: jnp.ndarray, n_samples: int) -> float:
        """
    ...
    def fit_gcv(self, X: Union[jnp.ndarray, np.ndarray], 
                y: Union[jnp.ndarray, np.ndarray],
                alpha_range: Tuple[float, float] = (1e-8, 1e4),
                n_alphas: int = 50,
                method: str = "standard",
                optimize: bool = True) -> "RandomFourierFeaturesRidgeGCV":
        """
        ...
        alphas_grid = jnp.logspace(
            jnp.log10(alpha_range[0]), 
            jnp.log10(alpha_range[1]), 
            n_alphas
        )
    ...
    def fit_gcv_with_path(self, X: Union[jnp.ndarray, np.ndarray],
                          y: Union[jnp.ndarray, np.ndarray],
                          alpha_range: Tuple[float, float] = (1e-8, 1e4),
                          n_alphas: int = 100,
    ...
    def plot_gcv_path(self, path_info: dict, save_path: str = None):
        ...
        ax = axes[0, 0]
        ax.semilogx(path_info['alphas'], path_info['gcv_scores'], 'b-', linewidth=2)
        ...
        ax = axes[0, 1]
        ax.loglog(path_info['alphas'], path_info['train_errors'], 'g-', linewidth=2)
        ax.axvline(path_info['alpha_opt'], color='r', linestyle='--')
        ax.set_xlabel('Regularization α')
        ...
        ax = axes[1, 0]
        ax.semilogx(path_info['alphas'], path_info['effective_dof'], 'm-', linewidth=2)
        ax.axvline(path_info['alpha_opt'], color='r', linestyle='--')
        ax.axhline(path_info['dof_opt'], color='r', linestyle=':', 
        ...
        # Plot 4: GCV vs DOF
        ax = axes[1, 1]
        ax.plot(path_info['effective_dof'], path_info['gcv_scores'], 'k-', linewidth=2)
        ...
        plt.suptitle('GCV Regularization Path Analysis', fontsize=14, fontweight='bold')
        plt.tight_layout()
