...
class TestRidge(ut.TestCase):
    def test_Ridge(self):
        ...
        fit_obj = ns.RidgeClassifier(lambda1 = 0.025, lambda2 = 0.5, 
                             n_hidden_features=5, n_clusters=0)

        fit_obj2 = ns.RidgeClassifier(lambda1 = 0.01, lambda2 = 0.01, 
                             n_hidden_features=10, n_clusters=2)
        
        fit_obj3 = ns.RidgeClassifier(lambda1 = 0.025, lambda2 = 0.05, 
                             n_hidden_features=5, n_clusters=0)

        fit_obj4 = ns.RidgeClassifier(lambda1 = 0.001, lambda2 = 0.01, 
                             n_hidden_features=10, n_clusters=2)
        ...
        self.assertTrue(
                ...
                & np.allclose(fit_obj3.predict(Z_test)[0], 2)
                & np.allclose(fit_obj4.predict(Z_test)[0], 2))
