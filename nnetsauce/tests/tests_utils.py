...
class TestUtils(ut.TestCase):
    ...
    def test_crossprod(self):
        A = 2 * np.eye(4)
        mo.crossprod(A)
        self.assertTrue(np.allclose(mo.crossprod(A), 4 * np.eye(4)))

    def test_tcrossprod(self):
        A = np.array([[1, 2], [3, 4]])
        B = np.array([[4, 3], [2, 1]])
        self.assertTrue(
            np.allclose(mo.tcrossprod(A, B), np.array([[10, 4], [24, 10]]))
            & np.allclose(mo.tcrossprod(A), np.array([[5, 11], [11, 25]]))
        )
    ...
    def test_cbind(self):
        ...
        B = np.array([[4, 3], [2, 1]])
        self.assertTrue(
            np.allclose(mo.cbind(A, B), np.array([[1, 2, 4, 3], [3, 4, 2, 1]]))
        )
    ...
    def test_inv_penalized_cov(self):
        ...
        self.assertTrue(
            np.allclose(
                ...
                np.array([[0.1207683, 0.04333116], [0.04333116, 0.15787413]]),
            )
            & np.allclose(
                lmf.inv_penalized_cov(X),
                np.array([[0.12244009, 0.04456527], [0.04456527, 0.16060273]]),
            )
        )
    ...
    def test_MTS_reformat_response(self):
        np.random.seed(123)
        X = np.random.rand(5, 2)
        self.assertAlmostEqual(ts.reformat_response(X, 2)[1], 0.28613933495)
    ...
    def test_beta_Sigma_hat_rvfl(self):
        ...
        fit1 = lmf.beta_Sigma_hat_rvfl(
            X=X, y=y, s=s, sigma=sigma, fit_intercept=True, return_cov=True
        )

        fit2 = lmf.beta_Sigma_hat_rvfl(
            X=X, y=y, s=s, sigma=sigma, fit_intercept=True, return_cov=False
        )

        fit3 = lmf.beta_Sigma_hat_rvfl(
            X=X, y=y, s=s, sigma=sigma, fit_intercept=False, return_cov=True
        )

        fit4 = lmf.beta_Sigma_hat_rvfl(
            X=X, y=y, s=s, sigma=sigma, fit_intercept=False, return_cov=False
        )

        fit5 = lmf.beta_Sigma_hat_rvfl(
            X=X,
            y=y,
            X_star=X,
            s=s,
    ...
    def test_beta_Sigma_hat_rvfl2(self):
        ...
        fit1 = lmf.beta_Sigma_hat_rvfl2(
            X=X, y=y, sigma=sigma, fit_intercept=True, return_cov=True
        )

        fit2 = lmf.beta_Sigma_hat_rvfl2(
            X=X, y=y, sigma=sigma, fit_intercept=True, return_cov=False
        )

        fit3 = lmf.beta_Sigma_hat_rvfl2(
            X=X, y=y, sigma=sigma, fit_intercept=False, return_cov=True
        )

        fit4 = lmf.beta_Sigma_hat_rvfl2(
            X=X, y=y, sigma=sigma, fit_intercept=False, return_cov=False
        )

        fit5 = lmf.beta_Sigma_hat_rvfl2(
            X=X, y=y, X_star=X, sigma=sigma, fit_intercept=True, return_cov=True
        )

        fit6 = lmf.beta_Sigma_hat_rvfl2(
            X=X,
            y=y,
            X_star=X,
            sigma=sigma,
