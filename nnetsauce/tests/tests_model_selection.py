...
class TS_Split(ut.TestCase):
    def test_tsplit(self):
        X = np.array(
            [
                [4, 5, 6, 1, 0, 2, 7, 9, 3, 8],
                [3.1, 3.5, 1.0, 2.1, 8.3, 1.1, 8.9, 0.3, 0.2, 8.7],
            ]
        ).T
        ...
        for train_index, test_index in tscv.split(
            X, initial_window=5, horizon=3, fixed_window=False
        ):
            0
            # print("TRAIN:", train_index, "TEST:", test_index)

        for train_index2, test_index2 in tscv.split(
            X, initial_window=5, horizon=3, fixed_window=True
        ):
            0
