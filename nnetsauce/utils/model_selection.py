...
def cross_val_score(estimator, X, 
        ...
        if isinstance(scoring, str):
            ...
            def err_func(X_test, X_pred, scoring):
                if (estimator.replications is not None) or (estimator.type_pi == "gaussian"): # probabilistic
        ...
        for train_index, test_index in iterator:
            ...
            if isinstance(X, pd.DataFrame): 
                ...
                estimator.fit(X[train_index, :], 
                              xreg=xreg, **kwargs)    
                X_test = X[test_index, :]          
            X_pred = estimator.predict(h=int(len(test_index)), 
                                       level=level, **kwargs)                    
