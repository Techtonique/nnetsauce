...
def quasirandom_search(func, ubound, lbound, n_points=1000, tol=1e-4):
    
    err = np.abs(0)    
    d = len(ubound)

    assert d == len(lbound), "ubound and lbound must have the same length"
    
    sobol_seq_choices = np.transpose(generate_sobol2(n_dims=d, 
                                                     n_points=n_points)) 
    
    x_choices = (ubound - lbound) * sobol_seq_choices + lbound                    
...
if __name__== "main":
    ...
    from sklearn.datasets import make_classification, load_digits, load_breast_cancer, load_wine, load_iris
    from sklearn.model_selection import train_test_split
    from time import time
    import matplotlib.pyplot as plt
    
    wd="/Users/moudiki/Documents/Python_Packages/nnetsauce"
    chdir(wd)
...
#                    max_iter=100, tol=1e-6) 
    ...
    minimize(fun = rosen, x0 = np.array([1.3, 0.7, 0.8, 1.9, 1.2])).x
    
    minimize(fun = rosen, x0 = np.array([1.3, 0.7, 0.8, 1.9, 1.2]), 
             method = 'BFGS')
    
    ns.utils.optim.quasirandom_search(func = rosen, 
                  ubound = np.repeat(-1.5, 5), 
                  lbound = np.repeat(1.5, 5), tol=1e-4)