...
def beta_hat(x, y, lam=None):
    ...
    if lam is None:
        return np.dot(inv_penalized_cov(x), 
                  mo.crossprod(x, y))
    else:
...
def beta_Sigma_hat(X=None, y=None, 
                   fit_intercept=False,
...
# without intercept! without intercept! without intercept!
def beta_Sigma_hat_rvfl2(X=None, y=None, 
                         Sigma=None, sigma=0.05,
                         fit_intercept=False,
    ...
    if (X is not None) & (y is not None):
        ...
        if fit_intercept == True:
            ...
            X = mo.cbind(np.ones(n), X)
            Cn = la.inv(np.dot(Sigma, mo.crossprod(X)) + (sigma**2)*np.eye(p + 1))
