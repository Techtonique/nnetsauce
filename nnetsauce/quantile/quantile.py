...
class QuantileRegressor(BaseEstimator, 
    ...
    def __init__(self, base_regressor, 
                 level=95, 
                 score="predictions"):
        assert score in ("predictions", "residuals", "conformal"),\
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if self.score in ("predictions", "residuals"):          
            ...
            base_predictions = self.base_regressor_.predict(X_calib)  # These are calibration predictions
            scoring_residuals = y_calib - base_predictions
            self.scoring_residuals_ = scoring_residuals            
            # Update base_predictions to use training predictions for optimization
            base_predictions = self.base_regressor_.predict(X_train)  # <-- Add this line
        else:
            raise ValueError("Invalid argument 'score'")
