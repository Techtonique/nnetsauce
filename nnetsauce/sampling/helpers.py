...
def subsample_indices(data, method='random', fraction=0.1, label_col=None, n_components=10, eps=0.5, min_samples=5, random_state=42):
    """
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if label_col is not None:
            if not isinstance(label_col, int) or label_col < 0 or label_col >= data.shape[1]:
                raise ValueError("For NumPy arrays, 'label_col' must be a valid column index.")
    ...
    if method == 'random':
        ...
        subsample_indices = rng.choice(data.shape[0], size=int(data.shape[0] * fraction), replace=False)
        ...
        _, _, _, subsample_indices = train_test_split(
            np.arange(data.shape[0]), labels, test_size=fraction, stratify=labels, random_state=random_state
        )
        return subsample_indices.tolist()
        ...
        for cluster in np.unique(clusters):
            ...
            subsample_indices.extend(rng.choice(cluster_indices, size=subsample_size, replace=False))
        ...
        if len(valid_clusters) == 0:
            raise ValueError("DBSCAN identified only noise points. Adjust `eps` or `min_samples`.")
        ...
        raise ValueError("Invalid sampling method. Choose from 'random', 'stratified', 'gmm', 'dbscan', or 'optimal_transport'.")
