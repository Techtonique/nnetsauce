...
class RidgeRegressor(Base, RegressorMixin):
    ...
    def __init__(self, lambda_=0.0, n_hidden_features=0, activation_name="relu",
                 a=0.01, nodes_sim="sobol", bias=True, dropout=0, direct_link=True,
                 n_clusters=0, cluster_encode=True, type_clust="kmeans",
                 type_scaling=("std", "std", "std"), col_sample=1, row_sample=1,
                 seed=123, backend="cpu"):
        super().__init__(
        ...
        if np.isscalar(self.lambda_):
            ...
            for lambda_ in self.lambda_:
                ...
                try:
                    ...
                    gcv = np.sum((y[:, np.newaxis] - y_pred)**2) / (n - np.sum(d2/div))**2
