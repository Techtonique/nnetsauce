...
{
  if (!inherits(x, "MTS"))
    stop("input object must be generated by nnetsauce$MTS")

  if (!is.null(x$start_input) & !is.null(x$start_input))
  {
    y <- ts(x$df_[, selected_series],
            start = x$start_input,
            frequency = x$frequency_input)
  } else {
    warning("object has no 'start_input' and 'frequency_input'")
    y <- ts(x$df_[, selected_series])
  }
